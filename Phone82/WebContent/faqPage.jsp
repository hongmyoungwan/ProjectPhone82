...
<%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt" %>
   ...
   <fmt:parseNumber var="endPage" integerOnly="true" value="${startPage+perBlock-1}"></fmt:parseNumber>
	...
	<c:if test="${endPage>totalNum}">
 	...
 	<c:if test="${totalCount==0}">1</c:if>
 	
 	
 	<c:if test="${totalCount!=0}">
 		<c:if test="${startPage==1}">&lt;&lt;&nbsp;</c:if>
 		<c:if test="${startPage!=1}">
 				<a href='FAQListServlet?
 				perPage=${perPage}
 		...
 		<c:if test="${curPage!=1}">
			...
			<a href='FAQListServlet?perPage=${perPage}&searchValue=${dto.searchValue}&curPage=${status.index}'>${status.index}</a>&nbsp;
			</c:if>
 			</c:forEach>&nbsp;
