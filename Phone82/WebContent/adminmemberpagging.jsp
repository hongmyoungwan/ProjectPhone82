...
<%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt" %>
   ...
   <c:set var="totalCount" value="${dto.getTotalCount()}"></c:set>
   	...
   	<a href="MemberManagementServlet?curPage=${paggingIdx}&curIdx=${curIdx}"> &lt; </a>
   	<c:forEach begin="${paggingIdx+1}" end="${totalNum+1}" varStatus="status"> 
   			...
   			<c:if test="${curPage==status.index}">
   				...
   				<c:if test="${curPage!=status.index}">
   				  ...
   				  <a href="MemberManagementServlet?curPage=${status.index}&curIdx=${curIdx}">${status.index}</a>&nbsp;
   			</c:if>
   		
   	</c:forEach>
   		<a href="MemberManagementServlet?curPage=${1+(pageIndex+1)*totalNum}&curIdx=${curIdx}"> &gt; </a>
